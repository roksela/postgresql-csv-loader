...
class TestCsvLoader(unittest.TestCase):
    ...
    _csv_filename_1 = "resources/stackoverflow_survey_results_public_sample.csv"
    _csv_filename_2 = "resources/simple_table.csv"
    _database_host = "localhost"
    _database_port = 5440
    _database_name = "tests"
    _user = "tests"
    ...
    def test_original_headers(self):
        ...
        original_headers = loader._read_headers(self._csv_filename_1)
        self.assertEqual(original_headers, self._original_headers)

    def test_headers(self):
        loader = self._get_loader()
        original_headers = loader._read_headers(self._csv_filename_1)
        headers = loader._unify_headers(original_headers)
        self.assertEqual(headers, self._unified_headers)
    ...
    def test_load_data(self):
        loader = self._get_loader()
        loader.load_data(self._csv_filename_1)

        result = self._check_count(self._table_name_1)
        self._drop(self._table_name_1)
        self.assertEqual(result, self._csv_1_record_count)

    def test_load_data_2_files(self):
        loader = self._get_loader()
        loader.load_data(self._csv_filename_2)
        loader.load_data(self._csv_filename_1)

        result = self._check_count(self._table_name_1)
        self.assertEqual(result, self._csv_1_record_count)
        result = self._check_count(self._table_name_2)
        self.assertEqual(result, self._csv_2_record_count)
        self._drop(self._table_name_1)
        self._drop(self._table_name_2)

    def test_load_data_append(self):
        loader = self._get_loader()
        loader.load_data(self._csv_filename_1, create_table=True)
        loader.load_data(self._csv_filename_1, create_table=False)

        result = self._check_count(self._table_name_1)
        self._drop(self._table_name_1)
        self.assertEqual(result, 2 * self._csv_1_record_count)

    def _get_loader(self):
        return CsvLoader(self._database_host, self._database_port, self._database_name, self._user)

    def _check_count(self, table_name):
        connection = connect(dbname=self._database_name, user=self._user, password=None,
    ...
    # for assertions
    _csv_1_record_count = 30
    _csv_2_record_count = 5
    _table_name_1 = "csv_stackoverflow_survey_results_public_sample"
    _table_name_2 = "csv_simple_table"

    _original_headers = ['Respondent', 'Professional', 'ProgramHobby', 'Country', 'University', 'EmploymentStatus',
    ...
    _unified_headers = ['respondent', 'professional', 'program_hobby', 'country', 'university', 'employment_status',
